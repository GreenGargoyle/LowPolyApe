...
// and make sure they exist. Validation layers should only be enabled on non-release builds.
    ...
    // Surface creation.
    LPADEBUG("Creating Vulkan surface...");
    if (!platform_create_vulkan_surface(plat_state, &context))
    {
        LPAERROR("Failed to create platform surface!");
        return FALSE;
    }
    LPADEBUG("Vulkan surface created.");

    // Device creation.
    if (!vulkan_create_device(&context))
    {
        LPAERROR("Failed to create device.");
        return FALSE;
    }
...
void vulkan_renderer_backend_shutdown(renderer_backend* backend)
{
    ...
    LPADEBUG("Destroying Vulkan device...");
    vulkan_device_destroy(&context);

    LPADEBUG("Destroying Vulkan surface...");
    if (context.surface) 
    {
...
i32 find_memory_index(u32 type_filter, u32 property_flags)
{
    ...
    LPAWARN("Unable to find suitable memory type!");
    return -1;
}