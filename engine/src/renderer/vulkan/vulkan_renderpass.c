...
)
{
    ...
    // Input from a shader (Input).
    subpass.inputAttachmentCount = 0;
    subpass.pInputAttachments = 0;

    // Attachments used for multisampling colour attachments (Resolve).
    subpass.pResolveAttachments = 0;

    // Attachments not used in this subpass, but must be preserved for the next (Preserve).
...
)
{
    ...
    lpazero_memory(clear_values, sizeof(VkClearValue) * 2);
