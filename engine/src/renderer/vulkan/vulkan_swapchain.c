...
)
{
    ...
    {
        LPAFATAL("Failed to acquire swapchain image!");
        return FALSE;
    }
...
)
{
    ...
    {
        // Swapchain is out of date, suboptimal, or a frambuffer resize has occurred. Trigger swapchain recreation.
        vulkan_swapchain_recreate(context, context->framebuffer_width, context->framebuffer_height, swapchain);
    }
    else if (result != VK_SUCCESS)
    {
        LPAFATAL("Failed to present swap chain image!");
    }
}
...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain)
{
    ...
    swapchain_extent.width = LPACLAMP(swapchain_extent.width, min.width, max.width);
    swapchain_extent.height = LPACLAMP(swapchain_extent.height, min.height, max.height);
    ...
    if (!swapchain->images)
    {
        swapchain->images = (VkImage*)lpaallocate(sizeof(VkImage) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    if (!swapchain->views)
    {
        swapchain->views = (VkImageView*)lpaallocate(sizeof(VkImageView) * swapchain->image_count, MEMORY_TAG_RENDERER);
    }
    ...
    if (!vulkan_device_detect_depth_format(&context->device))
    {
        ...
        LPAFATAL("Failed to find a supported format!");
    }
    ...
    LPAINFO("Swapchain created successfully.");
}
